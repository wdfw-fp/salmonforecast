...
source("R/archive/evaluate_forecasts2.r")
source("R/one_step_ahead.r")
source("R/rolling_perf.r")
...
if(find_best ==T){
  ...
  saveRDS(best_covariates,"tests/outputs/best_covariates_OPI_preseason_ensemble.rds")
}

best_covariates<-readRDS("tests/outputs/best_covariates_OPI_preseason_ensemble.rds")



ml<-lapply(best_covariates[[1]],#[best_covariates[[2]]$model_num[1:num_models]],
...
if(fit_one_step){
  ...
  write.csv(results,"tests/outputs/forecasts_ensemble.csv")

  save(results,file="tests/outputs/forecasts_ensemble.rds")
}else{
  results<-read_csv("tests/outputs/forecasts_ensemble.csv")
}
...
## rolling_year_window<-15
rp<-rolling_perf(results,dat,rolling_year_window,3,TY_ensemble,ml)
# rp$performance


ens<-ensemble(rp$all_mods %<>% group_by(year) %>% mutate(rank=rank(MAPE)) %>% ungroup,dat,TY_ensemble,k,slide=15,num_models,stack_metric)
